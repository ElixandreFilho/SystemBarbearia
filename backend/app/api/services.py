...
async def create_service(
    ...
    db.add(service)
    record_audit(db, request, admin.id, "SERVICE_CREATED", "Service", metadata=payload.model_dump())
    await db.commit()
...
async def update_service(
    ...
    record_audit(db, request, admin.id, "SERVICE_UPDATED", "Service", str(service.id), payload.model_dump(exclude_unset=True))
    await db.commit()
    await db.refresh(service)
    return service
...
async def deactivate_service(
    ...
    record_audit(db, request, admin.id, "SERVICE_DEACTIVATED", "Service", str(service.id))
    await db.commit()
